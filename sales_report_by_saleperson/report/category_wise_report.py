...
class ReportSalesSalespersonWise(models.AbstractModel):
                    ...
                    for p in data2:
                        ...
                        for k in data3:
                            ...
                            user = self.env['account.invoice'].search(
                                [('id', '=', k.invoice_id.id), ('write_uid', '=', id), ('discount_type', '!=', 'fixed'),
                                 ('origin', '=', k.origin), ('write_date', '>=', start_date),
                                 ('write_date', '<=', end_date), ('state', '=', 'paid')])

                            print "Product:", k.name
                            user_flag = False
                            for u in user:
                                if u.write_uid.id == id and u.discount_type == 'none':
                                    user_flag = True
                                    print user_flag
                                    print "userrrr:", u.write_uid.id
    ...
    def render_html(self, docids, data=None):
        ...
        print report
        lines = self.prepare_productcate_line_by_date(docs.start_date, docs.end_date, docs.categ_id.id, docs.user.id)
        print lines
